...
const MyAppointments = () => {
  ...
  // const refund = async (appointmentId) => {
  //   try {
  //     const { data } = await axios.post(
  //       backendUrl + "/api/user/refund-payment",
  //       { appointmentId },
  //       { headers: { token } }
  //     );
  //     if (data.success) {
          ...
          >
            ...
            <div className="flex flex-col gap-2 justify-end text-sm text-center">
              {!item.cancelled && item.payment && !item.isCompleted &&(
                <button className="sm:min-w-48 py-2 border rounded cursor-not-allowed text-stone-500 bg-indigo-50">
                  Paid
                </button>
              )}
